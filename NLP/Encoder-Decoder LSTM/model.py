...
class Model:
    ...
    def train_step(self, dataset):
        ...
        for batch in dataset:
            ...
            self.opt.zero_grad()
            outputs = self.model(src, trg).to(self.device)
            loss = self.loss(outputs, trg)
            loss.backward()
            ...
            target = i2w(outputs.argmax(1), self.vocab, target=True)
    ...
    def validation_step(self, dataset):
        ...
        with torch.no_grad():
            for batch in dataset:
                src = batch.src.to(self.device)
                trg = batch.trg.to(self.device)
                outputs = self.model(src, trg, force_ratio=1).to(self.device)
                loss = self.loss(outputs, trg)
                epoch_loss.append(loss.item())
